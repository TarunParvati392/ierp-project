...
const FaceLockUpdate = () => {
  ...
  const [lastUpdated, setLastUpdated] = useState(user?.facelockUpdateAt || null);
...
const captureFaceEmbedding = async () => {
  ...
  return Array.from(detection.descriptor); //Convert Float32 Array Normal Array
};
...
const handleUpdateFace = async () => {
  ...
  try{
    ...
    const res = await api.post('/facelock/update-facelock', {
  ...
  return (
    <div className={`${Styles.card} space-y-4 rounded-xl`}>
      ...
      {/* Right Side: Rules */}
      <div className="text-red-500 text space-y-2 mmd:w-1/3">
